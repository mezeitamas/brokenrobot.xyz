...
- **The Playwright MCP** (`@playwright/mcp`, from the `frontend-toolkit` plugin) runs on the **host** with system Chrome
  ...
  devcontainer suite.
- **The `chrome-devtools` and `terraform` MCP servers** share that boundary: `chrome-devtools` (also from
  the `frontend-toolkit` plugin) drives host Chrome like Playwright; `terraform` runs as a short-lived
  Docker container (image pulled daemon-side) that queries the public Terraform Registry. Like all MCP
  servers they're launched by Claude Desktop, not the bash sandbox.
