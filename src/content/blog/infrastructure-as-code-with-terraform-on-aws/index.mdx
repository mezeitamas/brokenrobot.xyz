...
_GitOps_ is a modern approach to _continuous delivery_ and _infrastructure management_ that leverages Git as the single source of truth. So we can safely say that without _GitOps_ there is no effective _Infrastructure as Code_.
...
Let's check how the improved development workflow looks like for this website after introducing Terraform, using Terraform Cloud and adding automation to applying the infrastructure changes.
