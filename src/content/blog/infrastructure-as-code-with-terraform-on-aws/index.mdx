...
tags: ['infrastructureascode', 'iac', 'terraform', 'terraformcloud', 'aws', 'staticwebsite', 'hosting', 'brokenrobot']
---
...
However as you see on the diagram one part of the development workflow is still manual, and that is the infrastructure changes. Let's dive a bit deeper into the current workflow.
...
-   **Automated CI/CD pipeline**: After the code is pushed, the CI/CD pipeline is automatically triggered. The pipeline handles the following steps:
    -   **Building the website**: The source code is compiled or processed, using a static site generator.
...
_Mutable_ infrastructure refers to an infrastructure that can be altered even after its initial provisioning. This offers development teams the flexibility to make on-the-fly adjustments to the infrastructure as needed.
...
_Infrastructure provisioning_ and _configuration management_ tools are both essential components of _IaC_ and automation in IT environments, but they serve different purposes and address different stages of the infrastructure lifecycle.
...
-   **Configuration style**: Whether the tool follows a _declarative_ approach, where we define the desired end state and the tool figures out the steps to achieve it, or an _imperative_ approach, where we explicitly outline the steps needed to reach the desired configuration.
...
_Configuration management_ tools take over after the infrastructure is provisioned, ensuring that the systems are properly configured, software is installed, and services are running as expected. They focus on maintaining and enforcing the desired state of the infrastructure over time.
...
In this blog post we don't discuss in details the _configuration management_ tools, as the focus is on the _infrastructure provisioning_.
...
Let's see how the development workflow looks like after introducing Terraform, using Terraform Cloud and adding automation to applying the infrastructure changes.
...
Let's dive a bit deeper into the improved and automated workflow.
...
In this blog post, we went through the concept of _Infrastructure as Code (IaC)_ and discussed its benefits and the available tools for managing infrastructure via code. We also explored the advantages of using _IaC_ to automate the provisioning and management of infrastructure resources.
