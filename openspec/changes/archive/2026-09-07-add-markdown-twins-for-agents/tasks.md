...
- [x] 1.1 Add a transform module under `src/utils/` that parses a post's raw MDX with satteri's
...
- [x] 1.2 Build the identifier → specifier map from the import statements the `mdxjsEsm` nodes carry —
      ...
      images live in another post's directory, resolves its three too. Rewrite native Markdown images
      through the same lookup: `beyond-tabs-and-spaces-finding-a-balance-in-coding-conventions` embeds
      its five diagrams as `![alt](./coding-conventions-*.svg)` with no component and no import, and
      the spec requires those targets absolute too. An image URL that is already absolute passes
      through untouched.
- [x] 1.3 Add the loud-failure guard, covering both content faults: if the walk ends with any
...
- [x] 2.1 Create `src/pages/blog/[...slug].md.ts` as a static endpoint with `getStaticPaths()` over
...
- [x] 2.2 Build the processed-image lookup with an eager `import.meta.glob` over the blog tree's image
      files and resolve each body image to an absolute URL against the configured `site`. The glob is
      itself an import of every asset, which is what makes Astro emit the base file the twin links —
      without it only the hashed responsive variants a `<Picture>` renders exist. Verify the five
      image-bearing posts' twins contain `![alt](https://www.brokenrobot.xyz/...)` with no unresolved
      `{identifier}` placeholder and no relative path, and that each URL exists under `dist/_astro/`.
- [x] 2.3 Add `yaml` (2.9.0, exact-pinned) as a dependency and emit the twin's frontmatter from the
...
- [x] 2.4 Assemble the body: `# <title>` as its first line, then the transformed prose, then the
...
- [x] 3.1 Extract twin-URL construction and the copyright/attribution sentence into one shared
...
- [x] 3.2 Repoint the post entries in `src/pages/llms.txt.ts` at the twins, using the shared
...
- [x] 3.3 Add two links to `ArticleLayout`'s `seo` slot, alongside the existing RSS alternate link:
...
- [x] 4.1 Add a Playwright spec that, for every post, requests `/blog/<slug>/index.md` and asserts a
...
- [x] 4.2 Extend that spec to pin the fenced code sample in
...
- [x] 4.3 Assert discovery from both entry points: a post's HTML page exposes its `alternate`
      Markdown link and its `describedby` link to `/llms.txt`, and `/llms.txt` lists that post by its
      `.md` URL.
- [x] 4.4 Assert the twins stay out of the sitemap, pinning the exclusion the design relies on so a
      future integration change cannot silently start indexing them. Read it over HTTP as
      `/sitemap-0.xml` — the same `dist/sitemap-0.xml` the preview server hands out — so the whole
      spec stays on the served-site layer.

## 5. Build-output audit

- [x] 5.1 Create `scripts/check-markdown-twins.mjs` following `scripts/check-third-party-resources.mjs`
...
- [x] 5.2 Add the `twins:check` npm script and run it in the pipeline's Build job immediately after
      `thirdparty:check`. Verify `npm run twins:check` passes after `npm run build`, and that the
      Build job runs it.
- [x] 5.3 Exclude `.md` from `scripts/check-third-party-resources.mjs`, with a comment recording why:
...
- [x] 5.4 Register `twins:check` in `docs/development/checks.md` — its own section under **Build
      ...
      still link to it rather than repeating it. The skill's `evals/evals.json` did enumerate the
      gate and state a count; it is rewritten to name the steps without a number, since the count
      this change makes wrong is exactly the part that goes stale.

## 6. Documentation

- [x] 6.1 Add a short note to `docs/tech-stack.md` under **Build & deployment**, beside the existing
