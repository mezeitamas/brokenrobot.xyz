...
- Add a **light/dark theme**: a CSP-safe inline script sets `data-theme` before paint (no flash), and a **Preact island** toggle in the header persists the reader's choice (`localStorage`) and honors `prefers-color-scheme`. Migrate `BaseLayout` off the current `class="scheme-light"`.
