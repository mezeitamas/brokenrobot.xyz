...
- Porting the prototype's React/JSX or its in-prototype "Tweaks" panel.
...
- **No-flash theme init via inline head script.** An `is:inline` `<script>` in `<head>` resolves theme (localStorage → `prefers-color-scheme` → light) and sets `document.documentElement.dataset.theme` before paint. The CSP already allows `script-src 'self' 'unsafe-inline'`, so an inline script is compliant; no inline `on*` attributes are used. _Why:_ only reliable way to avoid FOUC on a static site with no SSR. _Alt:_ nonce-based — unnecessary given current CSP.
- **Toggle wiring without inline handlers.** The header renders a `<button>`; a small `is:inline` script attaches a `click` listener (via `addEventListener`), flips `dataset.theme`, persists to `localStorage`, and updates the button's icon + `aria-pressed`/`aria-label`. _Why:_ CSP-safe (no `onclick`), progressive (button is inert without JS but theme still resolves).
